...
console.log("🔌 새로운 파일 업로드 WebSocket 연동 테스트 시작...");
...
// 고유한 테스트 파일 생성 (타임스탬프 포함)
const timestamp = Date.now();
const testFileContent =
  `WebSocket Upload Progress Test File - ${timestamp}\n` +
...
console.log("📁 새로운 테스트 파일 생성 완료:", testFilePath);
...
socket.on("connect", async () => {
  ...
  socket.emit("join_chatroom", {
    chatroomId: "5bd4c44f-ac2c-4117-ba40-d3362244a2a5",
  });
...
async function testFileUpload() {
  try {
    console.log("📤 새로운 파일 업로드 시작...");

    const form = new FormData();
    form.append("file", fs.createReadStream(testFilePath));
    form.append("displayName", `WebSocket Test File ${timestamp}`);
    form.append("accessType", "PRIVATE");
    form.append("sessionName", `WebSocket Test Session ${timestamp}`);
    form.append("createThread", "false");
    // WebSocket 브로드캐스트를 위해 chatroomId 추가 (실제 채팅방 ID 사용)
    form.append("chatroomId", "5bd4c44f-ac2c-4117-ba40-d3362244a2a5");
...
function printTestResults() {
  ...
  // WebSocket 이벤트 분석
  const uploadProgressEvents = websocketEvents.filter(
    (e) =>
      e.event.includes("upload") ||
      e.event.includes("progress") ||
      e.event.includes("file")
  );
  const errorEvents = websocketEvents.filter((e) => e.event === "error");
  const successEvents = websocketEvents.filter(
    (e) => e.event.includes("success") || e.event.includes("complete")
  );

  console.log(`\n📈 이벤트 분석:`);
  console.log(`   📤 업로드 관련 이벤트: ${uploadProgressEvents.length}개`);
  console.log(`   ❌ 에러 이벤트: ${errorEvents.length}개`);
  console.log(`   ✅ 성공 이벤트: ${successEvents.length}개`);

  console.log(
    `\n🎯 테스트 성공 여부: ${
      websocketEvents.length > 0 ? "✅ 성공" : "❌ 실패"
    }`
  );
